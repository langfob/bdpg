---
title: "BDPG preliminary analysis"
...
```{r breakIntoRelatedGroupsOfColumns}
...
#  Examine NA values in working data

### Missing values for some APP numSpp and numPU

As far as I can see viewing the working_data data frame, if one of these 2 values is missing, the other one is too.

So, look for which rows have them missing and see how many there are.

That turns out to be a number I've seen before, which I think happens to be the number of CORs.  So, see if that's true, and it is.  This only happens for COR values.  
...
notAlwaysNAbutAlwaysBothIfOne = c ('rsp_app_num_spp', 'rsp_app_num_PUs')

missingAppPUsppIdxs = which (is.na (working_data$rsp_app_num_spp))
length (missingAppPUsppIdxs)

plyr::count (msa_dt$rsp_cor_or_app_str [missingAppPUsppIdxs])

```

### Missing values for over and/or under cost err fracs wrt possible

This one is more complicated.  It's probably got something to do with an if statement in cases where there is an overcost or an undercost.
...
missingOverIdxs = which (is.na (working_data$rs_over_opt_cost_err_frac_of_possible_overcost))

missingUnderIdxs = which (is.na (working_data$rs_under_opt_cost_err_frac_of_possible_undercost))

missingBothIdxs = 
    which ((is.na (working_data$rs_over_opt_cost_err_frac_of_possible_overcost)
            & 
            is.na (working_data$rs_under_opt_cost_err_frac_of_possible_undercost)))

missingOneButNotBothIdxs = 
    which (xor (
        is.na (working_data$rs_over_opt_cost_err_frac_of_possible_overcost),
        is.na (working_data$rs_under_opt_cost_err_frac_of_possible_undercost)))
...
```{r plotRedundancyVsNotHugeErrMag}
...
```{r plotRedundancyVsNotHugeErrOut}
