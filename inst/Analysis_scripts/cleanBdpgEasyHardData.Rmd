...
easyHard_results_dir = "/Users/bill/D/Projects/ProblemDifficulty/Results/FullEasyHardV1/"
...
na_counts <- sapply (easyHard_df, function(y) sum(is.na(y)))    #  Returns a vector w/ named elements
glimpse (na_counts)
...
```{r combineEasyHard}
easyHard_df = bind_rows (easy_df, hard_df,  .id = "id")
#glimpse (easyHard_df)
    #  Convert dplyr's automatically generated dataset ids of "1" and "2" to 
    #  "Easy" and "Hard".
easyHard_df$id = sapply (easyHard_df$id, function (x) if (x == "1") "Easy" else "Hard")
```
...
getColsWithNonZeroNAct (easy_df)
getColsWithNonZeroNAct (hard_df)
getColsWithNonZeroNAct (easyHard_df)
```
...
easyHard_df %>% 
    mutate (cln_mod = "keep") -> easyHard_df
glimpse (easyHard_df)
```
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rsp_app_num_spp) -> app_num_spp_na_lines
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rsp_app_num_PUs) -> app_num_PUs_na_lines
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rsp_app_num_PUs) -> app_num_spp_and_PUs_na_lines
...
easyHard_df %>%
    ...
    mutate (rsp_app_num_PUs = ifelse (is.na (rsp_app_num_PUs), rsp_num_PUs, rsp_app_num_PUs)) -> easyHard_df
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rsp_allow_imperfect_wrap) %>%            
    group_by (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label) %>% 
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rsp_wrap_is_imperfect) %>%            
    group_by (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label) %>% 
    summarise (n_per_group = n()) %>% 
    glimpse
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rs_over_opt_cost_err_frac_of_possible_overcost) %>%            
    group_by (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label) %>% 
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rs_under_opt_cost_err_frac_of_possible_undercost) %>%            
    group_by (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label) %>% 
...
easyHard_df %>%
    ...
    select (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label, rs_over_opt_cost_err_frac_of_possible_overcost, rs_under_opt_cost_err_frac_of_possible_undercost) %>%     group_by (id, rsp_cor_or_app_str, rsp_base_wrap_str, rsp_combined_err_label) %>% 
...
missingOverIdxs = which (is.na (easyHard_df$rs_over_opt_cost_err_frac_of_possible_overcost))

missingUnderIdxs = which (is.na (easyHard_df$rs_under_opt_cost_err_frac_of_possible_undercost))

missingBothIdxs = 
    which ((is.na (easyHard_df$rs_over_opt_cost_err_frac_of_possible_overcost)
            & 
            is.na (easyHard_df$rs_under_opt_cost_err_frac_of_possible_undercost)))

missingOneButNotBothIdxs = 
    which (xor (
        is.na (easyHard_df$rs_over_opt_cost_err_frac_of_possible_overcost),
        is.na (easyHard_df$rs_under_opt_cost_err_frac_of_possible_undercost)))
...
easyHard_df %>% 
    mutate (emag = rsr_COR_euc_out_err_frac / 
                   rsp_euc_realized_Ftot_and_cost_in_err_frac) -> easyHard_df
...
if (dim (easy_app_nan_mag_lines) [1] != 0)
    {
    removed_easyHard_df_lines = NULL
    removed_easyHard_df_lines = save_removed_lines (removed_easyHard_df_lines, 
                                               easy_app_nan_mag_lines, 
                                               "removeAppMagNaNs")
    cat ("\n\n\n\nremoved_easyHard_df_lines\n\n")
    glimpse (removed_easyHard_df_lines)
    }

    #  Now, remove those lines from the original data set.
easyHard_df %>%
    filter (! (id == "Easy" & rsp_cor_or_app_str == "APP" & is.nan (emag))) -> easyHard_df
cat ("\n\n\n\neasyHard_df after easy_app_nan_mag_lines removed\n\n")
glimpse (easyHard_df)
```
...
emag = easyHard_df$rsr_COR_euc_out_err_frac / 
       easyHard_df$rsp_euc_realized_Ftot_and_cost_in_err_frac
emag [is.na (emag)] = 0

easyHard_df$err_mag = emag
```
...
sppPUsum = easyHard_df$rsp_num_spp + easyHard_df$rsp_num_PUs
easyHard_df$sppPUsum = sppPUsum

sppPUprod = easyHard_df$rsp_num_spp * easyHard_df$rsp_num_PUs
easyHard_df$sppPUprod = sppPUprod

cat (length (easyHard_df$sppPUsum))
cat (length (easyHard_df$sppPUprod))
```

#  Write the full clean data set to a csv file

```{r saveFullClnData}
print (cln_combinedEasyHard_path)
print (rs_name)
print (suffix)
base_path = cln_combinedEasyHard_path
outfile = paste0 (base_path, rs_name, suffix)
print (outfile)

write_output_data (easyHard_df, rs_name, cln_combinedEasyHard_path, suffix)
```
