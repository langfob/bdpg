...
msa_dt %>%
    filter (is.na (rsp_app_num_spp) & is.na (rsp_app_num_PUs)) %>% 
    mutate (rsp_app_num_spp = rsp_num_spp, 
            rsp_app_num_PUs = rsp_num_PUs, 
            cln_mod = "setMissingAppNumSppPUtoCorValues")
#glimpse (msa_dt)
...
missingOverIdxs = which (is.na (msa_dt$rs_over_opt_cost_err_frac_of_possible_overcost))

missingUnderIdxs = which (is.na (msa_dt$rs_under_opt_cost_err_frac_of_possible_undercost))

missingBothIdxs = 
    which ((is.na (msa_dt$rs_over_opt_cost_err_frac_of_possible_overcost)
            & 
            is.na (msa_dt$rs_under_opt_cost_err_frac_of_possible_undercost)))

missingOneButNotBothIdxs = 
    which (xor (
        is.na (msa_dt$rs_over_opt_cost_err_frac_of_possible_overcost),
        is.na (msa_dt$rs_under_opt_cost_err_frac_of_possible_undercost)))
...
write_output_data (msa_dt, rs_name, cln_combinedEasyHard_path, suffix)
```
