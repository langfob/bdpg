...
- Feature preprocessing
    - Haven't done any standardization or logs or anything.  Not sure what's needed yet.
- Variable selection
    - Haven't done any removal of any variables yet.
- Bayes for things like beliefs about amount and type of input error?
- Why is regr.eval() mape always Inf in these experiments?  
    - Is it a bug or are there just some divide by 0 cases or something?
- Should add another LM/RF pair using problem size, input error (and maybe timings), since this is what people might *believe* that they'd know if even if they couldn't actually know the input errors.  
...
knitr::opts_chunk$set(echo = FALSE)    #TRUE)    #, message=FALSE)
```
...
##base_path = "/Users/bill/D/Projects/ProblemDifficulty/Results/ExpGen6basicVariants/Combined_batches_3_2/Clean/cln_exp."
base_path = "/Users/bill/D/Projects/ProblemDifficulty/Results/ExpGen6basicVariants/Combined_batches_3_4/Clean/cln_exp."
...
#DataExplorer::create_report (working_df)
```
...
```{r}
ggplot (data = working_df) + 
    geom_point (aes (x = rsp_euc_realized_Ftot_and_cost_in_err_frac, 
                     y = err_mag, 
#                     color = "red"
                    ), #id), 
                shape=".") + 
    ggtitle (paste0 ("Input error vs. Error Magnification\n", rs_name)) + 

    # xlab("Total Euc Input Error Frac") +
    # ylab("Err magnification") + 
    labs (x = "Total Euc Input Error Frac", y = "Err magnification") +
    
    theme_bw () +     #  To center the title
#    theme_minimal () +     #  To center the title
    theme (plot.title = element_text (hjust = 0.5))    #  To center the title
...
Note:  there seem to be lots of 0 magnifications.  Are these really 0 or are they replacements for something that were generated in the data cleaning?  

Or, since the current number of 0's is exactly 400 (exactly 10% of the current number of Wrap experiments), is it due to some particular input setting?

Need to go back and check this out.

```{r}
cat ("\nNumber of points with 0 error magnification = ", 
     length (which (working_df$err_mag == 0)), "\n")
```

Playing with 2 ways for how ggplot() can limit what values appear in the plot.
- Tried using ylim() to limit the axes, but it gave me a warning message like this:
    - Removed 3532 rows containing missing values (geom_point).
- Ended up ignoring the ylim and instead, selecting the points to use in the data argument.
    - Note that you can't do the subsetting of the data points in the aes(x=...) statement.  You have to do it in the data argument.
...
if (TRUE)
    ...
    ggplot (data = working_df [idxs_of_not_huge_err_mags, ]) + 
        geom_point (aes (x = rsp_euc_realized_Ftot_and_cost_in_err_frac, 
                         y = err_mag, 
#                         color = "red"
                         ), #id), 
                    shape=".") + 
        ggtitle (paste0 ("Input error vs. Error Mag < 10\n", rs_name)) + 
        xlab("Total Euc Input Error Frac") +
        ylab("Err magnification") + 
        theme_bw () +     #  To center the title
        theme (plot.title = element_text (hjust = 0.5))    #  To center the title
...
```{r funcsGgplotSignedErrFracsAndRepShortfalls}
#===============================================================================
...
```{r funcsGgplotRepShortfallVsSignedErrFracs}
#===============================================================================
...
```{r funcsForGgplotPossibleErrorFracs}
#===============================================================================
...
**MAY WANT TO REDO THIS SECTION USING THE corrr LIBRARY.  SEE:**
...
```{r selectOldCorrs, include=FALSE}
...
plot_pred_eval_for_train_and_test <- 
    ...
    {
    train_true_values = train_df [, pred_col_name_str]
    predicted = predict (fitted_model, train_df)
    if (must_specify_predictions_vector) 
        train_preds = predicted$predictions else 
        train_preds = predicted
    train_pred_true_df = 
        data.frame (ds_label = rep ("train", length (train_true_values)), 
                    true_values = train_true_values, 
                    pred_values = train_preds)

    cat ("\nEvaluate results of testing on ", "TRAIN", " data:\n")
    regrEvalResults = regr.eval (train_true_values, 
                       train_preds, 
                       #stats = c ("mae", "mse"),    #, "rmse", "mape"))
                       train.y = train_true_values)
    train_rmse = round (regrEvalResults ["rmse"], digits = 3)
    print (regrEvalResults)
    
    #----------

    test_true_values = test_df [, pred_col_name_str]
    predicted = predict (fitted_model, test_df)
    if (must_specify_predictions_vector) 
        test_preds = predicted$predictions else 
        test_preds = predicted
    test_pred_true_df = 
        data.frame (ds_label = rep ("test", length (test_true_values)), 
                    true_values = test_true_values, 
                    pred_values = test_preds)
    
    cat ("\nEvaluate results of testing on ", "TEST", " data:\n")
    regrEvalResults = regr.eval (test_true_values, 
                       test_preds, 
                       #stats = c ("mae", "mse"),    #, "rmse", "mape"))
                       train.y = test_true_values)
    test_rmse = round (regrEvalResults ["rmse"], digits = 3)
    print (regrEvalResults)
    ...
    combined_pred_true_df = rbind (train_pred_true_df, test_pred_true_df)
    
    ggplot (data = combined_pred_true_df) + 
    ...
    ggtitle (paste0 (rs_name, " - True vs. Pred for ", model_name_str, 
                              " using ", vars_used_str, 
...
old_plot_pred_eval_for_train_and_test <- 
    function (fitted_model, 
                model_name_str,                 #  e.g., "LM" 
                pred_col_name_str,              #  e.g., "rsr_COR_euc_out_err_frac" 
                pred_value_name_display_str,    #  e.g., "RS out err"
                train_df, 
                test_df, 
                vars_used_str, # NOT CURRENTLY USED
                must_specify_predictions_vector=FALSE)
    {           
    old_par = par ("mfrow")
    par (mfrow=c(1,2))
