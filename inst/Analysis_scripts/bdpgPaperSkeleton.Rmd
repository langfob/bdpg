...
- 1 Base problem generated using the XU_xxxx method and no input error
...
###  XU_xxxx base problem generation
...
The size and relative difficulty of the problem generated is determined by the choice of parameters in the procedure above.  We derived these parameters from 4 parameters in the XU_xxxx method described in [XuPaper].  The parameters there are driven by theoretical considerations to control problem difficulty and are as follows:
...
- Search for a curve that has a section that specifies roughly the same number of species that occur on exactly 2 patches as we have in our base XU_xxxx problem and then,
...
Given a desired solution fraction and an original XU_xxxx problem:
...
- No occurrence could be added to any patch in the XU_xxxx problem's independent set.  
    - This is probably slightly stricter than necessary, but is guaranteed not to violate the original solution.
- The first added occurrence for any new species had to go on a patch in the XU_xxxx solution so that the original solution was guaranteed to also be a solution for the wrapped problem.
...
The most important network metrics we computed were redundancy measures given by Latapy et al. in [LatapyPaper].  **We chose these measures because ...**

**...  definition of redundancy measures  ...**

These measures aren't available in either the `igraph` or `bipartite` packages in R.  The measures are complicated to compute and we relied on an R  implementation on a web page by XXX [LatapyCodeWebsite].  That implementation had a few errors when we tried to run it, so we modified that code to allow it to run.  The final R code is given in the xxxxx.R source file in the R package built for this paper, `bdpg` [bdpgGithub]. 
...
##  Problem difficulty - Input errors vs. output errors
...
**[Do we care about this in this paper?]**

## Robustness to uncertainty

None of the reserve selectors tested were particularly robust to input uncertainty.  Marxan's summed solution is an attempt to increase the robustness of Marxan to input errors but it was only a little better than Gurobi and Marxan itself.  The Zonation-like method also slightly outperformed Gurobi and Marxan in terms of robustness but was roughly equivalent to Marxan's summed solution.  All other greedy selectors performed poorly.   

**[although didn't one of the richness ones do better on hard problems?  can't remember...]**
...
- Need for a better shared method of collaboration and testing in reserve design
    - everybody recreates everything
    - no distributional information about any aspect of reserve design (costs, species, etc)
    - no repository of test problems to generate methods more widely everything's an anecdote, can't generalise (wink wink)
    - no notion of variance in problem difficulty
    - no re-usable, evaluated error models
    - no cost models
...
All data and R source code [Rdevelopers] for this paper are open source and available under the [xyz] license. The R code is at [bdpgGithub].  The data is at [someplace].
